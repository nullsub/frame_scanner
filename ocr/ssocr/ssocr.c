...
int main(int argc, char **argv)
{
  ...
  for(i=0; i<number_of_digits; i++) {
    ...
    if((digits[i].x2 != digits[i].x1) && (digits[i].y2-digits[i].y1)/(digits[i].x2-digits[i].x1) > one_ratio) {
      if(flags & DEBUG_OUTPUT) {
  ...
  for(d=0; d<number_of_digits; d++) {
    ...
    if((digits[d].digit == D_UNKNOWN) && !(digits[d].y2 == digits[d].y1 || digits[d].y2 == digits[d].y1) &&
